...
Self.prototype.buildLegacyWalletDataBlock = function (block, skipCoinbaseTransactions) {
  return new Promise((resolve, reject) => {
    ...
    this.getTransactionsByBlock(block.hash).then((transactions) => {
      ...
      if (obj.coinbaseTX) {
        if (obj.coinbaseTX.outputs.lenght === 0) {
          return reject(new Error('Internal data consistency error (NCTO)'))
        }
      }
      ...
      if (!obj.coinbaseTX && obj.transactions.length === 0) {
        return reject(new Error('Internal data consistency error (NCNT)'))
      }

      /* Loop throught he transactions and make sure that they have inputs
         and outputs otherwise we're missing key information  that makes
         a transaction viable */
      obj.transactions.forEach((txn) => {
        if (txn.inputs.length === 0 || txn.outputs.length === 0) {
          return reject(new Error('Internal data consistency error (NTIO)'))
        }
      })
    }).then(() => {
      return resolve(obj)
