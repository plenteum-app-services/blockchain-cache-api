...
Self.prototype.getRandomOutputs = function (amount, mixin) {
  ...
  return new Promise((resolve, reject) => {
    /* Standard psuedo random number generator */
    ...
    this._query([
      ...
      /* Populate a list of random outputs we'd like to use */
      const indexes = []
      while (indexes.length !== mixin) {
        var rnd = randomInt(maximum)
...
Self.prototype.getMixableAmounts = function () {
  return new Promise((resolve, reject) => {
    this._query([
      'SELECT `outputs`.`amount`,`timestamp`,`height`,`txnHash`,`hash` ',
      'FROM (SELECT `transaction_outputs`.`amount` FROM `transaction_outputs` ',
      'LEFT JOIN `transactions` ON `transactions`.`txnHash` = `transaction_outputs`.`txnHash` ',
      'WHERE `globalIndex` = 0 AND `transactions`.`timestamp` <> 0  ORDER BY `amount`) as `outputs` ',
      'LEFT JOIN (SELECT `transaction_outputs`.`amount`, `transactions`.`timestamp`, `blocks`.`height`, `transactions`.`txnHash`, `blocks`.`hash` ',
      'FROM `transaction_outputs` ',
      'LEFT JOIN `transactions` ON `transactions`.`txnHash` = `transaction_outputs`.`txnHash` ',
      'LEFT JOIN `blocks` ON `blocks`.`hash` = `transactions`.`blockHash` ',
      'WHERE `globalIndex` = 3 ',
      'ORDER BY `amount`) as `spendable_outputs` ON `spendable_outputs`.`amount` = `outputs`.`amount`'
    ].join(''), []).then((results) => {
      if (results.length === 0) {
        return resolve([])
      }
