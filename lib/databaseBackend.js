...
Self.prototype.legacyGetWalletSyncData = function (startHeight, startTimestamp, blockHashCheckpoints) {
  const that = this
  return new Promise(async function (resolve, reject) {
    ...
    var blocks
    try {
      blocks = await that._query('SELECT `hash`, `height`, `timestamp` FROM `blocks` WHERE `height` >= ? ORDER BY `height` ASC LIMIT 100', [topHeight])
    } catch (e) {
      return reject(e)
    }
...
Self.prototype.getTransactionsByBlock = function (blockHash) {
  return new Promise((resolve, reject) => {
    this._query('SELECT * FROM `transactions` WHERE `blockHash` = ?', [blockHash]).then((rows) => {
    ...
    this.getTransactionsByBlock(block.hash).then((transactions) => {
      ...
      transactions.forEach((transaction) => {
        if (transaction.totalInputsAmount === 0) {
          ...
          obj.coinbaseTX.unlockTime = transaction.unlockTime
...
Self.prototype.getRandomOutputsForAmounts = function (amounts, mixin) {
  ...
  return new Promise((resolve, reject) => {
    ...
    /* Build the criteria of the SQL call to figure out what range
       of outputs we have to work with */
    var criteria = []
    ...
    this._query([
      ...
      'WHERE ' + criteria
    ].join(''), amounts).then(async function (results) {
      ...
      return that._query([
        ...
        'ORDER BY `amount` ASC, `globalIndex` ASC'
      ].join(''), randomValues)
    }).then((results) => {
      const response = []
      ...
      results.forEach((result) => {
        if (result.amount !== curObject.amount) {
